...
struct PatchLibraryItem: Identifiable {
    ...
    var displayName: String {
        ...
        if filename.hasPrefix("Souzaiosoficial EXTERNAL File (") || ["pescoco","peito","holograma"].contains(filename.lowercased()) {
            return filename
        }
        ...
        for url in urls where url.pathExtension.lowercased() == "3105" {
            do {
                ...
                if let contentKey = try PatchKeyStore.load(for: summary) {
                    ...
                    // Bundled packages (pescoco, peito, holograma, etc.) use internal password
                    // Imported user packages without known key stay locked
                    let name = url.deletingPathExtension().lastPathComponent.lowercased()
                    let isBundledName = name.hasPrefix("souzaiosoficial") || name == "pescoco" || name == "peito" || name == "holograma" || name.contains("file (")
                    guard isBundledName else {
                        decoded = nil
                        continue
                    }
                    do {
