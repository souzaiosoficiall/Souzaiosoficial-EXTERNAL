...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/SouzaiosoficialEXTERNAL-esign.XXXXXX")"
...
if [[ ! -x "$APP/Souzaiosoficial EXTERNAL" ]]; then
  echo "Error: expected original executable Souzaiosoficial EXTERNAL was not found in the app bundle." >&2
  exit 1
fi
